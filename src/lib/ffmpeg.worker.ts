  ...
  videoOut = "[vout]";
    ...
    if (hasEmojis) {
      emojiElements.forEach((el, i) => {
        ...
        const emojiScaled = `[em${i}scaled]`;
        const outLabel = `[vem${i}]`;
        if (el.rotation !== 0) {
          filterParts.push(
            `${inputLabel}scale=${scaledSize}:${scaledSize},rotate=${el.rotation}*PI/180:c=none:ow=rotw(${el.rotation}*PI/180):oh=roth(${el.rotation}*PI/180)${emojiScaled}`
          );
        } else {
          filterParts.push(`${inputLabel}scale=${scaledSize}:${scaledSize}${emojiScaled}`);
        }
        filterParts.push(`${videoOut}${emojiScaled}overlay=${pixelX}:${pixelY}${outLabel}`);
        videoOut = outLabel;
      });
    }
...
async function runExport(request: ExportRequest): Promise<ResultPayload> {
  ...
  try {
    ...
    // Write pre-rasterised emoji PNGs to FFmpeg VFS
    const validEmojiNames: string[] = [];
    const validEmojiEls: SerializedEmoji[] = [];
    for (let i = 0; i < (request.emojiOverlays?.length ?? 0); i++) {
      ...
      try {
        ...
        console.error(`[worker] failed to write emoji ${i}`, e);
      }
    }
